...
function initScrollAnimations() {
  ...
  document.querySelectorAll(
    ...
    '.auth-form-side, .marquee-section'
  ).forEach((el, i) => {
...
function initScrollSpy() {
  // Only works on pages that have sections with IDs (index.html)
  ...
  const spyObserver = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      ...
      navLinks.forEach(link => {
        ...
        // Remove active from all first, then set
        link.classList.remove('active');
        if (isMatch) link.classList.add('active');
      });
    });
  }, {
    // Fires when section crosses the middle of the viewport
